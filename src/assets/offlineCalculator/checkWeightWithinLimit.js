...
function checkWeightWithinLimit(data) {
  try {
    ...
    const limit = {
       ...
       */
      lower() {
        return config.value.weightLimits[data.patientSex].lower[ageInMonths];
      },
      /**
      ...
      upper() {
        let upper =
          config.value.weightLimits[data.patientSex].upper[ageInMonths];
